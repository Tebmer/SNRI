...
from utils.graph_utils import collate_dgl, move_batch_to_device_dgl, collate_dgl_train, move_batch_to_device_dgl_train
...
def main(params):
    ...
    adj_list, triplets, entity2id, relation2id, id2entity, id2relation, _,_,_,_ = process_files(params.file_paths, graph_classifier.relation2id)
    # ent2rels = {k: torch.LongTensor(v).to(device=params.device) for k, v in ent2rels.items()}
    # graph_classifier.ent2rels = ent2rels
...
if __name__ == '__main__':
    ...
    parser.add_argument('--disable_cuda', action='store_true', # default value is False
                        help='Disable CUDA')
    ...
    # parser.add_argument('--comp_hrt', type=str, default='TransE')
