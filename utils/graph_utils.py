...
def collate_dgl(samples):
    ...
    # batched_nei_rels_pos = nei_rels_poss
    # batched_nei_rels_pos = [sublist for sublist in nei_rels_poss]
    ...
    # batched_nei_rels_neg = [item for sublist in nei_rels_negs for item in sublist]
...
def collate_dgl_train(samples):
    ...
    return (batched_graph_pos, batched_graph_cor, r_labels_pos), g_labels_pos, (batched_graph_neg, r_labels_neg), g_labels_neg
...
def move_batch_to_device_dgl(batch, device):
    ...
    # ent2rels = {key: torch.LongTensor(value).to(device=device) for key, value in ent2rels.items()}
    # nei_rels_pos = torch.LongTensor(nei_rels_pos).to(device=device)
    # nei_rels_neg = torch.LongTensor(nei_rels_neg).to(device=device)
...
def move_batch_to_device_dgl_train(batch, device):
    ((g_dgl_pos, g_dgl_cor, r_labels_pos), targets_pos, (g_dgl_neg, r_labels_neg), targets_neg) = batch
    ...
    return ((g_dgl_pos, r_labels_pos), targets_pos, (g_dgl_neg, r_labels_neg), targets_neg, (g_dgl_cor, r_labels_pos))
